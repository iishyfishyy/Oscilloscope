...
function appController($mdSidenav, $scope, $timeout, frontendSocket) {
    ...
    frontendSocket.on('udp_message', function(data) {
        if(!pause) dv = new DataView(data.message);
        ...
        ctrl.channel.hwID = "0x" + (dv.getUint32(2, false).toString(16) + dv.getUint32(6, false).toString(16)).toUpperCase().padLeft(
            '0', 16);
        var t = new Date(parseInt(dv.getUint32(10, false)) * ctrl.ymax);
        ...
        link: function(scope, elem, attrs) {
            ...
            function Init() {
                ...
                chart = new Dygraph(elem[0], scope.app.channel.data, {
                    ...
                    valueRange: [ctrl.ymin, ctrl.ymax],
                    interactionModel: {
...
angular.module('griffinApp').directive('chart3d', function() {
    return {
            ...
            function(frontendSocket) {
                ...
                ctrl.updateAxisValues = function(){
                    ...
                    for(var i = ctrl.lines.canvas.offsetHeight, j = ctrl.lines.currentTop, k = 0; i > 0; i -= Math.floor(ctrl.lines.canvas.offsetHeight/num_divs), j -= max_y_value/num_divs, k++){
                        ...
                        if(text){
                            var out           = Math.round(ctrl.scaleBetween(j, ctrl.ymin, ctrl.ymax, -1, 1));
                ...
                ctrl.updateDivisionValues = function(){
                    if(!ctrl.manual_control){
                        ...
                        var top            = Math.round(ctrl.scaleBetween(ctrl.lines.currentTop, ctrl.ymin, ctrl.ymax, -1, 1));
                        var bottom         = Math.round(ctrl.scaleBetween(ctrl.lines.currentBottom, ctrl.ymin, ctrl.ymax, -1, 1));
                ...
                ctrl.calculateCursorValue = function(baseVal, divisionVal){
                    return (baseVal / (ctrl.ymax / (divisionVal * (ctrl.num_divs / 2)))).toFixed(2);
                }
        ...
        link: function(scope, elem, attrs) {
                ...
                scope.ctrl.ymin = parseInt(attrs.ymin);
                scope. ctrl.ymax = parseInt(attrs.ymax);
                ...
                scope.ctrl.lines = new WebGLLines({canvas: canvas, 
                                                   ...
                                                   ymin: scope.ctrl.ymin,
                                                   ymax: scope.ctrl.ymax});
                    ...
                    this.updateDivision = function(newValue){
                        if(angular.isDefined(newValue) && parseInt(newValue) > 0){
                            ...
                            if(this.divisionValueValid()){
                                var top     = Math.round(scope.ctrl.scaleBetween(scope.ctrl.lines.currentTop, scope.ctrl.ymin, scope.ctrl.ymax, -1, 1));
                                var bottom  = Math.round(scope.ctrl.scaleBetween(scope.ctrl.lines.currentBottom, scope.ctrl.ymin, scope.ctrl.ymax, -1, 1));
                                this.y_mult = (Math.abs(top - bottom) / scope.ctrl.num_divs) / this.y_division;
                            }
                        }
                ...
                scope.ctrl.last_points = null;
                scope.ctrl.temp_pts    = null;
                scope.$watch(attrs.updated, function() {
                    if(scope.ctrl.single && scope.ctrl.run){ 
                        scope.ctrl.run = !scope.ctrl.run;
                        pause = true;
                        scope.ctrl.lines.clearFlag = true;
                    }
                    if(scope.ctrl.run || scope.ctrl.single) {
                        scope.ctrl.temp_pts = scope.app.channel.data;
                        pause = false;
                    }
                    var num_channels   = 1;
                    if(!scope.ctrl.lines_init) { 
                        for(var i = 0; i < num_channels; i++){
                            scope.ctrl.channels.push(new Channel());
                            scope.ctrl.channels[i].color = scope.ctrl.rgbToHex(scope.ctrl.colors[i]);
                            scope.ctrl.channels[i].name  = scope.ctrl.channel_labels[i];
                            scope.ctrl.channels[i].wf_num= i;
                        }
                        scope.ctrl.lines.initChannels(num_channels, scope.ctrl.colors); 
                        scope.ctrl.lines_init = true; 
                    }
                    scope.ctrl.max_time = scope.ctrl.temp_pts.length;
                    scope.ctrl.lines.max_time = scope.ctrl.max_time;
                    var pts = [];
                    var sampleCnt = scope.ctrl.temp_pts.length;
                    var xadjust        = (scope.ctrl.lines.canvas.width / (sampleCnt - 1))*1.02;
                    var yadjust        = scope.ctrl.lines.canvas.height / 65536;
                    for(var i = 0; i < scope.ctrl.temp_pts.length; i++){
                        pts.push(scope.ctrl.temp_pts[i][0] * xadjust * scope.ctrl.x_mult);
                        pts.push(scope.ctrl.temp_pts[i][1] * scope.ctrl.channels[0].y_mult * yadjust + (
                                                            scope.ctrl.lines.canvas.height / 2));
                    }
                    if (num_channels != scope.ctrl.channels.length) location.reload();
                    scope.ctrl.lines.update(pts, 0, scope.ctrl.channels[0].visible);
                    if(scope.ctrl.single && !scope.ctrl.run) pause = false;
                    scope.ctrl.single = false;
                }, true);
                  ...
                  scope.ctrl.lines.canvas.addEventListener('keydown', function(e){
                    if (e.keyCode > 36 && e.keyCode < 41){ //keycodes for arrow keys, left:37 up:38 right:39 down:40
                      scope.ctrl.lines.keyAdjustCamera(e.keyCode, 
                                                       ...
                                                       scope.ctrl.scaleBetween(scope.ctrl.channels[scope.ctrl.currentChannel].y_division, -1, 1, scope.ctrl.ymin, scope.ctrl.ymax)
                                                       );
